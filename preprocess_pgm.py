...
class Paths():
    data_path = 'data'
    glove_path ='/home/akashyap/GLOVE/glove.6B.300d.pkl'
    ...
    pkl_path = 'dump'
    vocab_filename='paragram_dict_word_to_id_expanded_vanguard.pkl'
    word2pgm_filename = os.path.join(pkl_path,'paragram_dict_word_to_vec_expanded_vanguard.csv')
    response_vocab_filename='response_vocab_pgm.pkl'
    emb_matrix_filename='emb_matrix_pgm.pkl'
...
def get_pgm_matrix(filename, vocab):
    pgm_matrix = np.genfromtxt(filename)
    vocab_size = len(vocab)
    emb_matrix = np.zeros((vocab_size, 300))
    try:
        for token in vocab:
            emb_matrix[vocab[token],:] = pgm_matrix[vocab[token]]
    except ValueError:
        logger.info(token + "NOT IN PGM")
    return emb_matrix
...
def preprocess_data(flag,load_existing_vocab):
    ...
    pgm_matrix = get_pgm_matrix(Paths.word2pgm_filename, vocab)
    targets,utterances = [],[]
    ...
    for line in open(path):
        ...
        for token_id in token_ids:
            vec += pgm_matrix[token_id]
        vec = vec / num_tokens
        train_pgm_embeddings.append(vec)
        utterances.append(token_ids)
    valid_pgm_embeddings = []
    for line in open(Paths.valid_path):
        ...
        for token_id in token_ids:
            vec += pgm_matrix[token_id]
        vec = vec/num_tokens
        valid_pgm_embeddings.append(vec)
        valid_utterances.append(token_ids)
    num_ex = len(utterances)
    print num_ex
    return valid_utterances, valid_targets, utterances, targets, vocab, response_vocab, emb_matrix, train_pgm_embeddings, valid_pgm_embeddings
