...
class Params():
    ...
    model_path = 'models/'
    model_name = os.path.join(model_path, 'baseline_rnn_top5')
    max_grad_norm = 10
    init_scale = 0.1
    train_batches = None
    valid_batches = None
...
class RNNclassifier():
    def __init__(self, flag_is_train):
        ...
        self.flag_int = tf.placeholder(tf.float32, shape=[None], name="FLAG")
        ...
        # self.flag_one_hot = tf.cast(tf.arg_max(self.flag_one_hot, dimension=1, name='GET_RESPONSE_ID'),tf.int32)

        self.loss = tf.reduce_sum(tf.nn.softmax_cross_entropy_with_logits(predictions, self.flag_int, name="LOSS"))
        self.matches_topk = tf.nn.in_top_k(predictions, tf.cast(tf.int32, self.flag_int), 1, name='TOP_K')
...
def init_model(flag, load_from_existing):
    ...
    return sess, model_train, utterances
